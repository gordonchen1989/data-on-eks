...
#---------------------------------------------------------------
module "db" {
  count   = var.create_airflow_preq ? 1 : 0
...
resource "aws_efs_mount_target" "efs_mt" {
  count = var.create_airflow_preq ? length(var.eks_data_plane_subnet_secondary_cidr) : 0
...
data:
  ...
  # For git-sync v4
  GITSYNC_USERNAME: ${base64encode(aws_iam_service_specific_credential.git_credentials[0].service_user_name)}
  GITSYNC_PASSWORD: ${base64encode(aws_iam_service_specific_credential.git_credentials[0].service_password)}
YAML
}